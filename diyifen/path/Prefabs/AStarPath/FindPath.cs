...
public class FindPath : MonoBehaviour {
	//private Grid grid;
    public Grid grid;
	private Move moveItem;
	private bool isMove =false;

    List<Grid.NodeItem> Movepath = new List<Grid.NodeItem>();

    // Use this for initialization
    void Start () {
        //grid = GetComponent<Grid> ();
        moveItem = GetComponent<Move>();
		FindPathVoid();
    }

	public void FindPathVoid()
    {
        grid.getWall();
  ...
  //      FindingPath (grid.player.position, grid.destPos.position);
		...
		//if (Movepath.Count >= 2)
		//{
		//	Debug.Log("pos " + Movepath[Movepath.Count - 2].pos);
		//	moveItem.MoveToEnd(Movepath[Movepath.Count - 2].pos, 3f);
		//}
	}
	...
	void generatePath(Grid.NodeItem startNode, Grid.NodeItem endNode) {
        ...
        Queue<Grid.NodeItem> my_queue = new Queue<Grid.NodeItem>();
        if (endNode != null) {
