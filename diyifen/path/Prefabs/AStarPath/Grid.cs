...
public class Grid : MonoBehaviour {
	...
	void Awake() {
		// 初始化格子
		w = Mathf.RoundToInt(transform.localScale.x * MapSize);
		h = Mathf.RoundToInt(transform.localScale.y * MapSize);
	...
	public NodeItem getItem(Vector3 position) {
		int x = Mathf.RoundToInt (position.x) * MapSize;
		int y = Mathf.RoundToInt (position.y) * MapSize;
